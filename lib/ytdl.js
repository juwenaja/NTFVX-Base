...
const NTFVXLOG = (text, color) => {
	return !color ? chalk.yellow('[YTDL] ') + chalk.green(text) : chalk.yellow('[YTDL] ') + chalk.keyword(color)(text)
}

function post(url, formdata) {
    console.log((NTFVXLOG(Object.keys(formdata).map(key => `${key}=${encodeURIComponent(formdata[key])}`).join('&'))))
    return fetch(url, {
        method: 'POST',
        headers: {
            accept: "*/*",
