...
const cekWaktuTG = (ntfvx, _dir) => {
    setInterval(() => {
        ...
        if (position !== null) {
            ntfvx.sendMessage(_dir[position].id, `\`\`\`WAKTU HABIS\`\`\`\n\n*Jawaban:* ${_dir[position].jawaban}`, MessageType.text)
...
// Other Game
const addgameplay = (chatId, jawaban, expired, _db) => {
    let obi = { id: chatId, jawaban: jawaban, expired: Date.now() + toMs(`${expired}s`) }
    _db.push(obi)
}
const getJawabanGM = (chatId, _db) => {
    let found = false
    Object.keys(_db).forEach((i) => {
        if (_db[i].id === chatId) {
            found = i
...
const cekWaktuFam = (ntfvx, _dir) => {
    setInterval(() => {
        ...
        if (position !== null) {
            ...
            ntfvx.sendMessage(_dir[position].id, juwu, MessageType.text)
            _dir.splice(position, 1)
        }
    }, 1000)
}
